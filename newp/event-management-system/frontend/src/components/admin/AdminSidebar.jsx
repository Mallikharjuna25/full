...
import { FiHome, FiUsers, FiMonitor, FiBriefcase, FiLogOut, FiMenu, FiX } from 'react-icons/fi';
import useAuth from '../../hooks/useAuth';
import { useState } from 'react';
import { motion, AnimatePresence } from 'framer-motion';

const SidebarContent = ({ logout, navItems, setIsOpen }) => (
    <div className="flex flex-col h-full bg-surface-card border-r border-surface-border w-64 md:w-72">
        <div className="p-6">
            <h2 className="text-2xl font-heading font-bold text-gradient">Admin Pannel</h2>
        </div>
...
const AdminSidebar = () => {
    ...
    const navItems = [
        ...
        { name: 'Student Reviews', path: '/admin/dashboard/students', icon: <FiUsers /> },
        { name: 'Organizer Reviews', path: '/admin/dashboard/organizers', icon: <FiMonitor /> },
        { name: 'Colleges', path: '/admin/dashboard/colleges', icon: <FiBriefcase /> },
    ];
    ...
    return (
        <>
            ...
            <div className="hidden md:block h-screen fixed left-0 top-0">
                <SidebarContent logout={logout} navItems={navItems} setIsOpen={setIsOpen} />
            </div>
