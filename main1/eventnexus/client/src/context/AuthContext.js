...
export const AuthProvider = ({ children }) => {
    ...
    const loginStudent = async (email, password) => {
        ...
        try {
            ...
            console.error("Student Login Error:", error.response?.data);
    ...
    const loginCoordinator = async (collegeName, email, password) => {
        setAuthError(null);
        try {
            const { data } = await authAPI.loginCoordinator({
            ...
            console.error("Coordinator Login Error:", error.response?.data);
            let message = "Login failed";
            if (error.response?.data?.message) {
                message = error.response.data.message;
            } else if (error.response?.data?.errors && Array.isArray(error.response.data.errors) && error.response.data.errors.length > 0) {
    ...
    const registerStudent = async (formData) => {
        ...
        try {
            ...
            console.error("Student Registration Error:", error.response?.data);
    ...
    const registerCoordinator = async (formData) => {
        ...
        try {
            ...
            const { data } = await authAPI.registerCoordinator(sanitizedData);
            saveToStorage(data.token, data.user);
            setToken(data.token);
            setUser(data.user);
            return data.user;
        } catch (error) {
            console.error("Coordinator Registration Error:", error.response?.data);
            let message = "Registration failed";
            if (error.response?.data?.message) {
                message = error.response.data.message;
            } else if (error.response?.data?.errors && Array.isArray(error.response.data.errors) && error.response.data.errors.length > 0) {
